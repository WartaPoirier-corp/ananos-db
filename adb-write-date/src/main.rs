...
fn main() {
    ...
    let date = DbObject {
        type_info: db
            .get_type_info(adb::TypeId(0x00DA))
            .expect("Date type is not present, this program cannot insert it yet"),
        value: Arc::new(DbValue::Product {
            fields: vec![
                Arc::new(DbValue::U64(
                    matches
                        .value_of("year")
                        .expect("Please provide a year")
                        .parse()
                        .unwrap(),
                )),
                Arc::new(DbValue::U64(
